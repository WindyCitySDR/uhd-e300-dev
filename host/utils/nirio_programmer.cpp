...
int main(int argc, char *argv[])
{
    ...
    if (vm.count("status")){
		...
		uint32_t signature_offset = 0x100000;
		nirio_status_chain(dev_proxy.get_attribute(kRioFpgaDefaultSignatureOffset, signature_offset), status);
		std::string signature;
		for (int i = 0; i < 4; i++) {
			nirio_status_chain(dev_proxy.peek(signature_offset, attr_val), status);
