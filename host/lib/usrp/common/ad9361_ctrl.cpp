    ...
    {
        {
            ...
            if (not buff or buff->size() < 64)
                throw std::runtime_error("ad9361_ctrl_over_zc send timeout");
        ...
        }
        {
            ...
            if (not buff or buff->size() < 64)
                throw std::runtime_error("ad9361_ctrl_over_zc recv timeout");
...
class ad9361_ctrl_transport_sw_impl : public ad9361_ctrl_transport
{
public:
    ad9361_ctrl_transport_sw_impl(ad9361_product_t product, uhd::spi_iface::sptr ctrl_iface) :
        _spi_iface(ctrl_iface)
    {
        _device.product = product;
        _device.xact_iface = reinterpret_cast<void*>(_spi_iface.get());
    }
...
ad9361_ctrl_transport::sptr ad9361_ctrl_transport::make_software(ad9361_product_t product, uhd::spi_iface::sptr ctrl_iface)
{
    return sptr(new ad9361_ctrl_transport_sw_impl(product, ctrl_iface));
}

/***********************************************************************
 * AD9361 Control Class
 **********************************************************************/
...
public:
    ...
    ad9361_transaction_t do_transaction(const ad9361_transaction_t &request)
    {
        ...
        if (not error_msg.empty())
            throw uhd::runtime_error("ad9361 do transaction: " + error_msg);
