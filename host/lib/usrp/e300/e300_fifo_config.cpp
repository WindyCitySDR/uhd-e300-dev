...
public:
        ...
        _send_entries_in_use(0)
    {
        ...
        buff = ::mmap(NULL, _config.ctrl_length + _config.buff_length, PROT_READ|PROT_WRITE, MAP_SHARED, _fd, 0);
        if (buff == MAP_FAILED)
        {
            ::close(_fd);
        ...
        _ctrl_space = size_t(buff);
        _data_space = size_t(buff) + _config.ctrl_length;
    ...
    virtual ~e300_fifo_interface_impl(void)
    {
        ...
        ::munmap(buff, _config.ctrl_length + _config.buff_length);
        ::close(_fd);
    }
