...
# Parse options
parser = argparse.ArgumentParser(description='Process LVBITX.')
parser.add_argument('lvbitx-filename', type=str,
					help='Path to input LVBITX file.')
parser.add_argument('--uhd-images-path', dest='uhd-images-path', nargs=1, action='store', default='', type=str,
					help='Install location for UHD images.')
parser.add_argument('--merge-bin', dest='merge-bin', nargs=1, action='store', default='', type=str,
					help='Path to bin file that needs to be merged with the LVBITX before exporting. Not specifying this option disables merging.')
parser.add_argument("--output-bin", action="store_true", dest="out-bin", default=False,
					help="Generate a binary FPGA programming bitstream file.")
parser.add_argument("--output-lvbitx", action="store_true", dest="out-lvbitx", default=False,
					help="Generate an LVBITX FPGA programming bitstream file.")
parser.add_argument('--output-path', dest='output-path', nargs=1, action='store', default='', type=str,
					help='Output path for autogenerated files.')
args = vars(parser.parse_args())

# Args
input_filename = os.path.abspath(args['lvbitx-filename'])
output_path = os.path.abspath(args['output-path'][0]) if (args['output-path'] != '') else os.path.dirname(input_filename)
...
if (not os.path.isfile(input_filename)):
	...
	sys.exit(1)
if (args['merge-bin'] != '' and not os.path.isfile(os.path.abspath(args['merge-bin'][0]))):
	print 'ERROR: FPGA Bin File ' + args['merge-bin'][0] + ' could not be accessed or is not a file.'
	sys.exit(1)
if (not os.path.exists(output_path)):
	print 'ERROR: Output path ' + output_path + ' could not be accessed.'
	sys.exit(1)
if (args['out-lvbitx'] and input_filename == os.path.join(output_path, class_name + '.lvbitx')):
...
if (args['merge-bin'] != ''):
	with open(os.path.abspath(args['merge-bin'][0]), 'rb') as bin_file:
...
if (args['out-lvbitx'] and md5.new(bitstream).hexdigest() != root.find('BitstreamMD5').text):
	...
	sys.exit(1)
if (args['out-bin']):
	fpga_bin_file = open(os.path.join(output_path, class_name + '.bin'), 'w')
	fpga_bin_file.write(bitstream)
	fpga_bin_file.close()
	
# Save LVBITX
if (args['out-lvbitx']):
	tree.write(os.path.join(output_path, class_name + '_fpga.lvbitx'), encoding="utf-8", xml_declaration=True, default_namespace=None, method="xml")

# Save HPP and CPP
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'template_lvbitx.hpp'), 'r') as template_file:
    template_string = template_file.read()
with open(os.path.join(output_path, class_name + '_lvbitx.hpp'), 'w') as source_file:
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'template_lvbitx.cpp'), 'r') as template_file:
    template_string = template_file.read()
with open(os.path.join(output_path, class_name + '_lvbitx.cpp'), 'w') as source_file:
    source_file.write(template_string.format(**codegen_transform))
