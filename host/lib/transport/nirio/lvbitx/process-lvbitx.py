...
parser.add_argument('--output-lvbitx-path', dest='output-lvbitx', nargs=1, action='store', default='', type=str,
					help='Output path for autogenerated files.')
parser.add_argument('--output-src-path', dest='output-src', nargs=1, action='store', default='', type=str,
					help='Output path for autogenerated files.')
args = vars(parser.parse_args())

# Args
input_filename = os.path.abspath(args['lvbitx-filename'])
autogen_src_path = os.path.abspath(args['output-src'][0]) if (args['output-src'] != '') else os.path.dirname(input_filename)
autogen_lvbitx_path = os.path.abspath(args['output-lvbitx'][0]) if (args['output-lvbitx'] != '') else ''
...
if (not os.path.exists(autogen_src_path)):
	...
	sys.exit(1)
if (autogen_lvbitx_path != '' and input_filename == os.path.join(autogen_src_path, class_name + '.lvbitx')):
...
if (autogen_lvbitx_path != '' and md5.new(bitstream).hexdigest() != root.find('BitstreamMD5').text):
	...
	sys.exit(1)
if (args['out-bin']):
	fpga_bin_file = open(os.path.join(autogen_lvbitx_path, class_name + '.bin'), 'w')
	fpga_bin_file.write(bitstream)
	fpga_bin_file.close()
	
# Save LVBITX
if (autogen_lvbitx_path != ''):
	tree.write(os.path.join(autogen_lvbitx_path, class_name + '_fpga.lvbitx'), encoding="utf-8", xml_declaration=True, default_namespace=None, method="xml")
