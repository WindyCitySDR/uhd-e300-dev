...
const device_addr_t& loopback_test::run_test
(
    ...
    )
{
    ...
    _tx_thread.reset(new boost::thread(boost::bind(&loopback_test::_stream_tx, this, tx_transport.get(), &pkt_info, big_endian)));
    _rx_thread.reset(new boost::thread(boost::bind(&loopback_test::_stream_rx, this, rx_transport.get(), &pkt_info, big_endian)));
...
void loopback_test::_stream_tx(zero_copy_if* transport, vrt::if_packet_info_t* pkt_info, bool big_endian)
{
    while (not boost::this_thread::interruption_requested()) {
	    ...
	    // Add 'A's
            boost::uint32_t *data_buff = packet_buff + pkt_info->num_header_words32;
            memset((void *) data_buff, 0xAA, pkt_info->num_payload_bytes);
...
void loopback_test::_stream_rx(zero_copy_if* transport, const vrt::if_packet_info_t* exp_pkt_info, bool big_endian)
{
    while (not boost::this_thread::interruption_requested()) {
...
void loopback_test::_reset_counters(void)
{
    _num_tx_packets = 0;
    _num_rx_packets = 0;
    _num_tx_timeouts = 0;
...
void loopback_test::_initialize_chdr(
    zero_copy_if::sptr tx_transport,
    zero_copy_if::sptr rx_transport,
    boost::uint32_t sid,
    vrt::if_packet_info_t& pkt_info)
