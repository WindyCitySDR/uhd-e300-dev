...
static void register_neighbor(const int8_t ethno, const uint16_t seq, const struct ip_addr *node)
{
...
static void send_link_state_data_to_all_neighbors(const uint8_t ethno, const uint16_t seq, const void *buff, const size_t num_bytes)
{
    for (size_t i = 0; i < LS_MAX_NUM_NBORS; i++)
    {
        if (ls_neighbors[i].valid && !is_seq_expired(ls_neighbors[i].seq) && ls_neighbors[i].ethno == ethno)
        {
...
bool update_table(const uint16_t seq, const ls_data_t *ls_data)
{
...
static void handle_icmp_irq(
    ...
    case LS_ID_INFORM_DIRECT:
        ...
        {
            if (ls_data->node.addr == u3_net_stack_get_ip_addr(e)->addr) break;
        }

        if (update_table(seq, ls_data))
        {
